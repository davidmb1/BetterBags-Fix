...
function items:ParseItemLink(link)
  -- Protección inicial: link debe ser string válido
  if type(link) ~= "string" or link == "" then
    print("|cffff0000[BetterBags] Error: item link inválido recibido:|r", tostring(link))
    return {
      itemID = 0,
      enchantID = "",
      gemID1 = "",
  ...
  if type(rest) == "string" and rest ~= "" then
    if rest and rest:find(":") then
      numBonusIDs, rest = strsplit(":", rest, 2)
      if numBonusIDs ~= "" then
        local splits = tonumber(numBonusIDs)
        if splits then
          splits = splits + 1
  ...
  if type(rest) == "string" and rest ~= "" then
    ...
    if numModifiers ~= "" then
      local splits = tonumber(numModifiers)
      if splits then
        splits = (splits * 2) + 1
  ...
  if type(rest) == "string" and rest ~= "" then
    ...
    if relic1NumBonusIDs ~= "" then
      local splits = tonumber(relic1NumBonusIDs)
      if splits then
        splits = splits + 1
  ...
  if type(rest) == "string" and rest ~= "" then
    ...
    if relic2NumBonusIDs ~= "" then
      local splits = tonumber(relic2NumBonusIDs)
      if splits then
        splits = splits + 1
        relic2BonusIDs = strsplittable(":", rest, splits)
  ...
  if type(rest) == "string" and rest ~= "" then
    ...
    if relic3NumBonusIDs ~= "" then
      local splits = tonumber(relic3NumBonusIDs)
      if splits then
        splits = splits + 1
        relic3BonusIDs = strsplittable(":", rest, splits)
